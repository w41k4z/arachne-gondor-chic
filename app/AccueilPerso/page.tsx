...
const GondorChic = () => {
    ...
    const handleAddToChaudron = async () => {
        ...
        try {
            ...
            const userData = localStorage.getItem('gondorUser')
            let clientId = 'client123' // default value
            ...
            // Call ajouterAuPanier function
            const produitRetourne = await ajouterAuPanier(clientId, produitDuJour.id)
            
            // Update produitDuJour state with returned data
            setProduitDuJour(produitRetourne)
            
            console.log(`Added ${quantity} ${produitRetourne.libelle} to chaudron`)
        } catch (error) {
